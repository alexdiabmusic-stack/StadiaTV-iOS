...
struct NBAFixtureTests {
    @Test func scoreboardNormalization() throws {
        ...
        #expect(game.id.providerID == "0022500500")
...
@Suite("BasketballCourtCoordinateTransformer")
struct BasketballCourtCoordinateTransformerTests {
    @Test func normalizesTenthsOfFeetToFeet() {
        let shot = BasketballCourtCoordinateTransformer.normalize(xLegacy: 120, yLegacy: 230, distanceFeet: 26, made: true, value: 3)
        #expect(shot?.x == 12); #expect(shot?.y == 23)
    }
    @Test func nilWhenEitherCoordinateMissingEvenWithDistance() {
        #expect(BasketballCourtCoordinateTransformer.normalize(xLegacy: nil, yLegacy: 230, distanceFeet: 26, made: true, value: 3) == nil)
        #expect(BasketballCourtCoordinateTransformer.normalize(xLegacy: 120, yLegacy: nil, distanceFeet: 26, made: true, value: 3) == nil)
    }
    @Test func nilOnNonFinite() {
        #expect(BasketballCourtCoordinateTransformer.normalize(xLegacy: .nan, yLegacy: 230, distanceFeet: nil, made: true, value: 3) == nil)
        #expect(BasketballCourtCoordinateTransformer.normalize(xLegacy: 120, yLegacy: .infinity, distanceFeet: nil, made: true, value: 3) == nil)
    }
    @Test func rejectsRatherThanClampsOutOfEnvelope() {
        #expect(BasketballCourtCoordinateTransformer.normalize(xLegacy: 4000, yLegacy: 230, distanceFeet: nil, made: true, value: 2) == nil)
        #expect(BasketballCourtCoordinateTransformer.normalize(xLegacy: 0, yLegacy: -1000, distanceFeet: nil, made: true, value: 2) == nil)
    }
    @Test func zeroZeroSentinelRejectedUnlessNearRimDistance() {
        #expect(BasketballCourtCoordinateTransformer.normalize(xLegacy: 0, yLegacy: 0, distanceFeet: 22, made: true, value: 2) == nil)
        let dunk = BasketballCourtCoordinateTransformer.normalize(xLegacy: 0, yLegacy: 0, distanceFeet: 0, made: true, value: 2)
    ...
    @Test func distanceReconciliationRejectsInconsistentLabel() {
        ...
        #expect(BasketballCourtCoordinateTransformer.normalize(xLegacy: 120, yLegacy: 230, distanceFeet: 3, made: true, value: 3) == nil)
    }
    @Test func distanceReconciliationDerivesWhenAbsent() {
        let shot = BasketballCourtCoordinateTransformer.normalize(xLegacy: 0, yLegacy: 100, distanceFeet: nil, made: true, value: 2)
        #expect(shot?.distanceFeet == 10)
    }
    @Test func rejectsInvalidShotValue() {
        #expect(BasketballCourtCoordinateTransformer.normalize(xLegacy: 0, yLegacy: 100, distanceFeet: 10, made: true, value: 1) == nil)
    }
}

@Suite("BasketballPlayDescriptionBuilder")
struct BasketballPlayDescriptionBuilderTests {
    @Test func suppliedDescriptionReturnedVerbatim() {
        let (title, _) = BasketballPlayDescriptionBuilder.describe(type: .madeShot, description: "Curry 26' 3PT Jump Shot (12 PTS) (Green 3 AST)",
    ...
    @Test func composesOnlyWhenDescriptionAbsent() {
        let (title, _) = BasketballPlayDescriptionBuilder.describe(type: .madeShot, description: nil, playerName: "James", assistPlayerName: nil,
    ...
    @Test func freeThrowNeverClaimsMadeOrMissed() {
        let (title, _) = BasketballPlayDescriptionBuilder.describe(type: .freeThrow, description: nil, playerName: "Davis", assistPlayerName: nil,
    ...
    @Test func halftimeLabelDistinctFromOtherPeriodEnds() {
        let (halftime, _) = BasketballPlayDescriptionBuilder.describe(type: .periodEnd, description: nil, playerName: nil, assistPlayerName: nil,
        ...
        let (thirdEnd, _) = BasketballPlayDescriptionBuilder.describe(type: .periodEnd, description: nil, playerName: nil, assistPlayerName: nil,
    ...
    @Test func unknownTypeNeverDropped() {
        let (title, _) = BasketballPlayDescriptionBuilder.describe(type: .unknown("weird_event"), description: nil, playerName: nil, assistPlayerName: nil,
            blockPlayerName: nil, stealPlayerName: nil, teamTricode: nil, shotDistance: nil, period: 1, scoreHome: nil, scoreAway: nil)
        #expect(title == "Weird event")
    }
    @Test func subtitleOmitsScoreForNonScoringPlays() {
        let (_, subtitle) = BasketballPlayDescriptionBuilder.describe(type: .rebound, description: nil, playerName: "Davis", assistPlayerName: nil,
    ...
    @Test func priorityTable() {
        #expect(BasketballPlayDescriptionBuilder.priority(for: .madeShot, subType: nil) == .high)
        #expect(BasketballPlayDescriptionBuilder.priority(for: .flagrantFoul, subType: nil) == .high)
        #expect(BasketballPlayDescriptionBuilder.priority(for: .missedShot, subType: nil) == .medium)
        #expect(BasketballPlayDescriptionBuilder.priority(for: .rebound, subType: nil) == .compact)
        #expect(BasketballPlayDescriptionBuilder.priority(for: .turnover, subType: "shot clock") == .medium)
        #expect(BasketballPlayDescriptionBuilder.priority(for: .turnover, subType: "bad pass") == .compact)
    }
}
...
struct NBAPlayMapperTests {
    @Test func dedupesByGameIDAndActionID() throws {
        ...
        let gameID = try #require(BasketballGameID.validated("0022500500", league: .nba))
    ...
    @Test func sortStableAtEqualClock() {
        let gameID = BasketballGameID(league: .nba, providerID: "0022500500")
        ...
        let b = NBAPlayEvent(id: "a", gameID: gameID, actionNumber: 1, orderNumber: 1, period: 1, clockText: "12:00", type: .rebound,
            ...
            isFieldGoal: false, shot: nil, isPeriodBoundary: false, priority: .compact, assistPlayerName: nil, videoAvailable: false)
        let sorted = NBAPlayEvent.sorted([a, b])
...
struct NBADurationTests {
    ...
    @Test func subMinuteClockShowsTenths() {
        // Broadcast convention once under a minute remains — never truncated to
        // whole seconds, and never the raw "PT00M29.80S" ISO text (shared by both
        // NBA and WNBA via this one function).
        #expect(NBADuration.clockText("PT00M29.80S") == "0:29.8")
        #expect(NBADuration.clockText("PT00M05.30S") == "0:05.3")
    }
