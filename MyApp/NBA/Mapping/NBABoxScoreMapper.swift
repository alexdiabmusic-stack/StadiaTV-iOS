...
nonisolated enum NBABoxScoreMapper {
    static func players(_ teamRaw: NBAValue) -> [NBAPlayerGameLine] {
    ...
    static func teamStats(_ teamRaw: NBAValue) -> NBATeamStatLine? {
    ...
    static func onCourt(_ teamRaw: NBAValue) -> [Int] {
        teamRaw["players"].array.filter { $0["oncourt"].bool == true }.compactMap { $0["personId"].int }
    }
    /// This-game leaders — the highest currently-supplied value per stat, independently
    /// per category (a team's leading scorer need not be its leading rebounder).
