...
/// mirrors `EPLLegacyMapper`'s shape exactly. La Liga is a single-table league, so
/// `standings(_:)` takes one `SoccerStandingsTable` (EPL's signature), not MLS's
/// conference-split `[SoccerStandingsTable]`.
nonisolated enum LaLigaLegacyMapper {
    static let league = League(name: "La Liga", shortName: "La Liga", path: "soccer/esp.1", group: .soccer)
    ...
    static func match(_ match: SoccerMatch) -> Match {
        func side(_ state: SoccerTeamMatchState) -> TeamSide {
            ...
            return TeamSide(displayName: state.team.name, shortName: state.team.shortName, abbreviation: state.team.abbreviation,
                logoURL: LaLigaAssetResolver.shared.logoURL(teamID: state.team.id), score: state.score.map(String.init), record: nil, isWinner: isWinner,
                teamID: state.team.id, canonicalIDString: "team:\(league.bannerKey):laliga:\(state.team.id)")
        }
        ...
        result.canonicalID = "game:\(league.bannerKey):laliga:\(match.id)"
        return result
    }
    ...
    /// official/FotMob response arrives.
    static func seed(from match: Match) -> SoccerMatch? {
        ...
        return SoccerMatch(id: match.id, competitionID: String(LaLigaProviderConfiguration.competitionID), season: String(LaLigaSeasonResolver.startingYear(for: match.date)),
    ...
    static func team(_ team: SoccerTeam) -> Team {
        Team(id: team.id, displayName: team.name, shortDisplayName: team.shortName, abbreviation: team.abbreviation,
             logoURL: LaLigaAssetResolver.shared.logoURL(teamID: team.id), canonicalIDString: "team:\(league.bannerKey):laliga:\(team.id)")
    }

    /// La Liga is a single table (no conference split, Step 30) — one `StandingsGroup`.
    static func standings(_ table: SoccerStandingsTable) -> [StandingsGroup] {
        let rows = table.entries.sorted { ($0.overall.position ?? Int.max) < ($1.overall.position ?? Int.max) }.map { entry -> StandingRow in
            StandingRow(teamID: entry.team.id, displayName: entry.team.name, abbreviation: entry.team.abbreviation, logoURL: LaLigaAssetResolver.shared.logoURL(teamID: entry.team.id),
        ...
        // No documented live=true table (Step 30) — this mapper is only ever fed the
        // pre-match official table, so the name never says "Live".
        return [StandingsGroup(id: "laliga-official", name: "La Liga", rows: rows)]
    }
    ...
    static func athlete(_ player: SoccerRosterPlayer) -> RosterAthlete {
        ...
        result.canonicalID = "player:\(league.bannerKey):laliga:\(player.reference.id)"
        return result
    }
    ...
    static func athlete(id: String, name: String, jersey: String? = nil, position: String? = nil) -> RosterAthlete {
        ...
        result.canonicalID = "player:\(league.bannerKey):laliga:\(id)"
        return result
    }
}
