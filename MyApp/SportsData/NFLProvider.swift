...
struct NFLProvider: ScoreProvider, ScheduleProvider, GameDetailsProvider, PlayByPlayProvider, BoxScoreProvider, TeamProvider, StandingsProvider, RosterProvider, InjuryProvider {
    ...
    func schedule(for league: League, range: SportsDateRange) async throws -> BannerSchedule {
        ...
        return BannerSchedule(
            id: BannerEntityID(rawValue: "schedule:nfl:\(league.bannerKey):\(Int(range.start.timeIntervalSince1970)):\(Int(range.end.timeIntervalSince1970))"),
    ...
    func playByPlay(for league: League, gameID: BannerEntityID) async throws -> BannerPlayByPlay {
        ...
        let providerID = try resolvedNFLGameID(from: gameID)
        let detail = try await client.gameDetails(gameID: providerID)
        let plays = (detail.plays ?? []).enumerated().compactMap { index, play -> BannerPlay? in
            ...
            return BannerPlay(
                id: BannerEntityID(rawValue: "play:nfl:\(providerID):\(play.playID ?? String(index))"),
                sequence: play.sequence ?? index,
                period: period.map { BannerPeriod(number: $0, displayName: NFLPeriodFormatter.displayName(for: $0)) },
                clock: play.clock.map { BannerGameClock(displayValue: $0, remainingSeconds: nil, isRunning: nil) },
                text: text,
        ...
        return BannerPlayByPlay(id: BannerEntityID(rawValue: "pbp:nfl:\(providerID)"), gameID: gameID, plays: plays, provenance: DataProvenance(provider: .nfl, fetchedAt: Date(), providerEntityID: providerID, confidence: 0.78))
    }
    ...
    func boxScore(for league: League, gameID: BannerEntityID) async throws -> BannerBoxScore {
        ...
        return BannerBoxScore(id: BannerEntityID(rawValue: "boxScore:nfl:\(providerID)"), gameID: gameID, teamStats: stats, playerStats: [], provenance: DataProvenance(provider: .nfl, fetchedAt: Date(), providerEntityID: providerID, confidence: 0.78))
    }
    ...
    func standings(for league: League) async throws -> [BannerStandingGroup] {
        ...
        let rows = (response.weeks ?? []).flatMap { $0.standings ?? [] }.enumerated().compactMap { index, row -> BannerStanding? in
    ...
    func roster(for league: League, teamID: BannerEntityID) async throws -> BannerRoster {
        ...
        return BannerRoster(id: BannerEntityID(rawValue: "roster:nfl:\(teamID.rawValue)"), teamID: teamID, leagueID: SportsIdentityResolver.canonicalLeagueID(for: league), players: players, provenance: DataProvenance(provider: .nfl, fetchedAt: Date(), providerEntityID: providerTeamID, confidence: 0.82))
    }
    ...
    private func resolvedNFLGameID(from gameID: BannerEntityID) throws -> String {
    ...
    private func mapGame(_ dto: NFLGameDTO, league: League) -> BannerGame? {
        ...
        let status = BannerGameStatus(nflStatus: dto.status, start: start)
        let period = dto.status?.periodNumber.map { BannerPeriod(number: $0, displayName: NFLPeriodFormatter.displayName(for: $0)) }
        let clock = dto.status?.clockDisplay.map { BannerGameClock(displayValue: $0, remainingSeconds: nil, isRunning: status == .live) }
        return BannerGame(id: identityResolver.canonicalGameID(league: league, provider: .nfl, providerGameID: providerGameID, home: home, away: away, scheduledStart: start), leagueID: SportsIdentityResolver.canonicalLeagueID(for: league), scheduledStart: start, name: dto.name ?? "\(away.displayName) at \(home.displayName)", shortName: dto.shortName ?? "\(away.abbreviation) @ \(home.abbreviation)", status: status, statusDetail: NFLStatusFormatter.detail(status: status, dto: dto.status, start: start), homeTeam: home, awayTeam: away, score: BannerScore(home: (dto.homeScore ?? homeDTO.score).map(String.init), away: (dto.awayScore ?? awayDTO.score).map(String.init)), clock: clock, period: period, venue: dto.venue?.bannerVenue, broadcasts: dto.broadcasts?.compactMap { $0.bannerBroadcast } ?? dto.network.map { [BannerBroadcast(network: $0, type: "TV", countryCode: nil)] } ?? [], aliases: [ProviderEntityAlias(provider: .nfl, id: providerGameID)], provenance: DataProvenance(provider: .nfl, fetchedAt: Date(), providerEntityID: providerGameID, confidence: 0.86))
    }

    private func mapTeam(_ dto: NFLTeamDTO?, league: League) -> BannerTeam? {
    ...
    private func mapTeamStats(_ team: NFLTeamDTO?, side: String, league: League, gameID: BannerEntityID) -> BannerTeamStat? {
        ...
        return BannerTeamStat(id: BannerEntityID(rawValue: "teamStat:nfl:\(gameID.rawValue):\(side)"), teamID: mapped.id, seasonID: nil, stats: values, provenance: DataProvenance(provider: .nfl, fetchedAt: Date(), providerEntityID: team?.providerID, confidence: 0.76))
    }

    private func mapInjury(_ dto: NFLInjuryDTO, league: League) -> BannerInjury? {
...
struct NFLVenueDTO: Decodable, Sendable {
    ...
    var bannerVenue: BannerVenue? { guard name != nil || city != nil else { return nil }; return BannerVenue(id: id.map { BannerEntityID(rawValue: "venue:nfl:\($0)") }, name: name ?? "Venue", city: city, state: state, country: country, aliases: id.map { [ProviderEntityAlias(provider: .nfl, id: $0)] } ?? []) }
}
struct NFLBroadcastDTO: Decodable, Sendable {
    ...
    var bannerBroadcast: BannerBroadcast? { (name ?? network ?? channel).map { BannerBroadcast(network: $0, type: type ?? "TV", countryCode: countryCode) } }
}
...
enum NFLStatusFormatter { nonisolated static func detail(status: BannerGameStatus, dto: NFLStatusDTO?, start: Date) -> String { if let value = dto?.displayStatus ?? dto?.shortDescription ?? dto?.description, !value.isEmpty { return value }; if status == .live { return [dto?.periodNumber.map(NFLPeriodFormatter.displayName), dto?.clockDisplay].compactMap { $0 }.joined(separator: " · ") }; if status == .final { return "Final" }; return RelativeDateTimeFormatter().localizedString(for: start, relativeTo: Date()) } }
extension BannerGameStatus { init(nflStatus: NFLStatusDTO?, start: Date) { let text = nflStatus?.statusText ?? ""; if nflStatus?.isFinal == true || text.contains("final") || text.contains("complete") || text.contains("closed") { self = .final } else if nflStatus?.isLive == true || text.contains("live") || text.contains("progress") || text.contains("halftime") || text.contains("quarter") { self = .live } else if text.contains("postpon") { self = .postponed } else if text.contains("delay") { self = .delayed } else if text.contains("cancel") { self = .cancelled } else if start.timeIntervalSinceNow < 900 && start.timeIntervalSinceNow > -900 { self = .pregame } else { self = .scheduled } } }
