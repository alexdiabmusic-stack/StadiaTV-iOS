...
final class BasketballGameCenterViewModel {
    private(set) var snapshot: BasketballGameSnapshot?
    private(set) var errors: [String: String] = [:]
    private(set) var showingCache = false
    private(set) var isRefreshing = false
    var selectedTab: BasketballGameTab = .overview
    private let service: any BasketballGameCenterServing
    private let cache: BasketballGameCenterCache
    private var generation = UUID()
    private var failures = 0
    private var retryAfter: Date?

    init(service: any BasketballGameCenterServing, cache: BasketballGameCenterCache) {
    ...
    func run(gameID: BasketballGameID, active: Bool, seed: BasketballGame? = nil, fullRefresh: Bool = true) async {
        ...
        repeat {
            ...
            do {
                ...
                snapshot = BasketballGameCenterReducer.apply(update, to: old)
