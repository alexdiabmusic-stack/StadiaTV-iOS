...
/// `Team`, `StandingsGroup`, `RosterAthlete`, `AthleteOverview`) that `MatchesView`,
/// `HomeView`, `LiveView`, and the tvOS surfaces already consume via `SportsRepository`.
/// Every other native provider (MLB/NBA/NFL/NHL/F1) has its own equivalent mapper.
nonisolated enum EPLLegacyMapper {
    static let league = League(name: "Premier League", shortName: "EPL", path: "soccer/eng.1", group: .soccer)
    ...
    static func match(_ match: SoccerMatch) -> Match {
        func side(_ state: SoccerTeamMatchState) -> TeamSide {
            ...
            return TeamSide(displayName: state.team.name, shortName: state.team.shortName, abbreviation: state.team.abbreviation,
                logoURL: EPLAssetResolver.logoURL(teamID: state.team.id), score: state.score.map(String.init), record: nil, isWinner: isWinner,
                teamID: state.team.id, canonicalIDString: "team:\(league.bannerKey):epl:\(state.team.id)")
        }
        ...
        var result = Match(id: match.id, league: league, date: match.kickoff, name: "\(match.away.team.name) at \(match.home.team.name)",
            ...
            statusDetail: match.clock?.display ?? EPLLegacyMapper.pregameDetail(match.kickoff), home: side(match.home), away: side(match.away),
            broadcasts: [], venue: match.ground, liveContext: context)
        result.canonicalID = "game:\(league.bannerKey):epl:\(match.id)"
        return result
    }
    ...
    static func seed(from match: Match) -> SoccerMatch? {
        ...
        return SoccerMatch(id: match.id, competitionID: EPLSeasonResolver.competitionID, season: EPLSeasonResolver.seasonParameter(for: match.date),
    ...
    static func team(_ team: SoccerTeam) -> Team {
        Team(id: team.id, displayName: team.name, shortDisplayName: team.shortName, abbreviation: team.abbreviation,
             logoURL: EPLAssetResolver.logoURL(teamID: team.id), canonicalIDString: "team:\(league.bannerKey):epl:\(team.id)")
    }

    static func standings(_ table: SoccerStandingsTable) -> [StandingsGroup] {
        let rows = table.entries.sorted { ($0.overall.position ?? Int.max) < ($1.overall.position ?? Int.max) }.map { entry -> StandingRow in
            StandingRow(teamID: entry.team.id, displayName: entry.team.name, abbreviation: entry.team.abbreviation, logoURL: EPLAssetResolver.logoURL(teamID: entry.team.id),
        ...
        let name = table.isLive ? "Premier League (Live)" : "Premier League"
        return [StandingsGroup(id: "epl-\(table.isLive ? "live" : "official")", name: name, rows: rows)]
    }
    ...
    static func athlete(_ player: SoccerRosterPlayer) -> RosterAthlete {
        ...
        result.canonicalID = "player:\(league.bannerKey):epl:\(player.reference.id)"
        return result
    }
    ...
    /// `NBALegacyMapper.athlete(id:name:jersey:position:)`. Never navigates by name
    /// alone: `id` is always the provider's real player ID.
    static func athlete(id: String, name: String, jersey: String? = nil, position: String? = nil) -> RosterAthlete {
        ...
        result.canonicalID = "player:\(league.bannerKey):epl:\(id)"
        return result
    }
}
