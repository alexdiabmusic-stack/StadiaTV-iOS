...
struct NBAGameLeadersView: View {
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            if let leaders {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    NBAPlayerLink(personID: fallback.personID ?? 0, name: fallback.name ?? "Unknown player", league: league)
    ...
    private func row(_ leader: NBAStatLeader, label: String) -> some View {
        HStack {
            NBAPlayerLink(personID: leader.personID, name: leader.name, league: league)
            Spacer()
