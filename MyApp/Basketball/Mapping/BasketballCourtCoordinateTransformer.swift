...
/// NBA/WNBA's legacy `xLegacy`/`yLegacy` fields are already basket-origin, just in
...
/// Shared verbatim between leagues — pure math, no NBA-specific behavior. Not yet
/// confirmed against a real WNBA payload (Step 16 of the WNBA integration marks
/// shot-chart support optional/verified-required) — if WNBA's units or sentinel
/// convention differ, this degrades to dropping the point (never fabricating one),
/// same as it already does for NBA's own out-of-bounds/no-location cases.
nonisolated enum BasketballCourtCoordinateTransformer {
