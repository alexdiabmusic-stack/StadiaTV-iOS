...
struct BasketballPlayerLink: View {
    let personID: Int
    let name: String
    let league: League
    ...
    var body: some View {
        NavigationLink {
            PlayerDetailView(league: league, athlete: BasketballLegacyMapper.athlete(id: personID, name: name, jersey: nil, position: nil, config: config))
        } label: { Text(name).font(.headline).frame(minHeight: 44, alignment: .leading) }
        .buttonStyle(.plain)
    }
}

struct BasketballBoxScoreView: View {
    ...
    private func table(_ title: String, _ lines: [NBAPlayerGameLine]) -> some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            ForEach(lines) { line in
                VStack(alignment: .leading, spacing: 6) {
                    HStack {
                        BasketballPlayerLink(personID: line.personID, name: line.name, league: league, config: config)
