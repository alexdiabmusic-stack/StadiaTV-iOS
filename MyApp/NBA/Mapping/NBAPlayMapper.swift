...
nonisolated enum NBAPlayMapper {
    static func events(_ response: NBAPlayByPlayResponse, gameID: NBAProviderGameID) -> [NBAPlayEvent] {
        merge(response.actions, gameID: gameID)
    }
    static func eventsV3(_ response: NBAPlayByPlayV3Response, gameID: NBAProviderGameID) -> [NBAPlayEvent] {
        merge(response.actions.map { NBAValue.object($0) }, gameID: gameID)
    }
    private static func merge(_ actions: [NBAValue], gameID: NBAProviderGameID) -> [NBAPlayEvent] {
        ...
        return sorted(Array(mapped.values))
    }
    /// Shared with `NBAGameCenterReducer` so a merged play list is ordered identically
    /// everywhere — two independent comparators would eventually diverge.
    static func sorted(_ events: [NBAPlayEvent]) -> [NBAPlayEvent] {
        events.sorted { $0.sortKey == $1.sortKey ? $0.id < $1.id : $0.sortKey < $1.sortKey }
    }
    private static func build(_ raw: NBAValue, gameID: NBAProviderGameID, fallbackIndex: Int) -> NBAPlayEvent? {
        ...
        let shot = type.isShot
            ? NBACourtCoordinateTransformer.normalize(xLegacy: xLegacy, yLegacy: yLegacy, distanceFeet: shotDistance, made: type == .madeShot, value: shotValue)
            : nil
        ...
        let (title, subtitle) = NBAPlayDescriptionBuilder.describe(
        ...
        let id = "\(gameID.rawValue):\(actionID.map(String.init) ?? actionNumber.map(String.init) ?? "i\(fallbackIndex)")"
        return NBAPlayEvent(id: id, gameID: gameID, actionNumber: actionNumber, orderNumber: orderNumber ?? fallbackIndex,
            ...
            priority: NBAPlayDescriptionBuilder.priority(for: type, subType: subType), assistPlayerName: assistPlayerName,
